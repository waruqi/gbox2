...
# 	define G2_GL2x_VERTICES_GROW 				(256)
#else
# 	define G2_GL2x_VERTICES_GROW 				(512)
#endif

// the mcstack grow
#ifdef TB_CONFIG_BINARY_SMALL
# 	define G2_GL2x_MCSTACK_GROW 				(32)
#else
# 	define G2_GL2x_MCSTACK_GROW 				(64)
#endif
...
tb_handle_t g2_init(tb_handle_t context)
{
	...
	// alloc
	g2_gl2x_painter_t* gpainter = tb_malloc0(sizeof(g2_gl2x_painter_t));
	tb_assert_and_check_return_val(gpainter, TB_NULL);

	// init context
	gpainter->context = (g2_gl2x_context_t*)context;
	...
	// init vertices
	gpainter->vertices = tb_vector_init(G2_GL2x_VERTICES_GROW, tb_item_func_ifm(sizeof(tb_float_t) << 1, TB_NULL, TB_NULL));
	...
	// init stack
	gpainter->mcstack = tb_stack_init(G2_GL2x_MCSTACK_GROW, tb_item_func_ifm(sizeof(g2_gl2x_mcitem_t), TB_NULL, TB_NULL));
...
{
	g2_gl2x_painter_t* gpainter = (g2_gl2x_painter_t*)painter;
	if (gpainter)
	{
		// exit vertices
...
tb_void_t g2_load(tb_handle_t painter)
{
	...
	// init item
	g2_gl2x_mcitem_t* item = tb_stack_top(gpainter->mcstack);
...
tb_void_t g2_draw_path(tb_handle_t painter, tb_handle_t path)
{
	...
	if (g2_gl_path_make_fill((g2_gl_path_t*)path))
		g2_gl2x_fill_path(gpainter, (g2_gl_path_t const*)path);
}
...
tb_void_t g2_draw_rect(tb_handle_t painter, g2_rect_t const* rect)
{
	...
	// fill
	g2_gl2x_fill_rect(gpainter, rect);
}
...
tb_void_t g2_draw_circle(tb_handle_t painter, g2_circle_t const* circle)
{
	...
	// fill
	g2_gl2x_fill_circle(gpainter, circle);
}
tb_void_t g2_draw_ellipse(tb_handle_t painter, g2_ellipse_t const* ellipse)
{
	...
	// fill
	g2_gl2x_fill_ellipse(gpainter, ellipse);
}
tb_void_t g2_draw_triangle(tb_handle_t painter, g2_triangle_t const* triangle)
{
	...
	// fill
	g2_gl2x_fill_triangle(gpainter, triangle);
}
